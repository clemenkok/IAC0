...
Generally, std::unique_ptr accomplishes everything that std::auto_ptr is capable of, whilst std::auto_ptr is only used if you need to compiler C++98 code. **std::unique_ptr** is the most commonly used smart pointer, and is for **exclusive-ownership resource management.** They work the same as raw pointers, and for most operations, including **dereferencing,** they execute the same instructions.
